...
The gate is **lenient warn-only** — `/oracle-archive` only hard-blocks on uncommitted changes inside the change folder. Everything else (incomplete Progress, missing impl-review, status not in `{implemented, impl_reviewed}`) is surfaced as a warning followed by a confirmation prompt; the user can still archive.

After archiving, every other oracle skill refuses to write inside `context/archive/<...>/` (each guarded skill checks the resolved path prefix and aborts with a fixed message). Archived folders are read-only by convention.
...
Examples:
  /oracle-archive context-dir-restructure
  /oracle-archive @context/changes/oauth-login/
...
Commit or stash them first, then re-run /oracle-archive.
```
...
Either commit them first or `git reset` to unstage, then re-run /oracle-archive.
```
...
```
⚠ /oracle-archive warnings for <change-id>:
...
- `Resume implementation` (description: `Don't archive. Suggest /oracle-implement <change-id> next.`)
...
- If the user chooses **Resume implementation** → print `→ /oracle-implement <change-id>` and copy that to clipboard via `pbcopy 2>/dev/null || clip.exe 2>/dev/null || xclip -selection clipboard 2>/dev/null || true` (or `Set-Clipboard` on PowerShell) (best effort, cross-platform). STOP.
...
5. **Close the matching roadmap item** — best effort; this step never blocks, never rolls back, and never prompts. A roadmap is optional; most changes won't trace to one.
   ...
   5. **Match found** → apply the three edits below by modifying the file. Each is independent and best effort: if a target isn't where the `/oracle-roadmap` template puts it (hand-edited roadmap, older format), skip that sub-edit, keep going, and note what was skipped — never abort the archive over roadmap shape. Touch only the fields named here; leave `Outcome`, `Prerequisites`, `Parallel with`, `Risk`, etc. alone.
...
The folder is now read-only by convention. To start a new change: /oracle-new <new-id>
```

## Error handling

- Any unexpected filesystem error during the move leaves the source folder in place — the staged `change.md` edits land before the move, so on partial failure the user sees `status: archived` in `context/changes/<change-id>/change.md` but the folder is still in `context/changes/`. `/oracle-status` will surface this as a `status drift: archived in wrong folder` warning. Re-running `/oracle-archive` is safe: the resolution check at the top will detect `status: archived` and ask the user to inspect manually.
...
- Does not append SHAs to Progress items — `/oracle-implement` is the sole writer of the SHA suffix at phase end. The archive gate enforces SHA presence as a warn-only signal (see soft-warning check 4); it never rewrites a SHA-less row.
...
- Does not write to `context/archive/<...>/` after the move; archived folders are read-only by convention. Other oracle skills (`/oracle-research`, `/oracle-frame`, `/oracle-plan`, `/oracle-plan-review`, `/oracle-implement`, `/oracle-impl-review`, `/oracle-tdd`, `/oracle-goal-implement`) refuse when a resolved path starts with `context/archive/`.
- Does not unarchive. To revisit an archived change, open a new change with `/oracle-new` and reference the archived folder for context.
...
# /oracle-implement — Implement Plan
...
1. **Resolve the plan**:
   - If invoked as `/oracle-implement <change-id> [phase N]`, resolve to `context/changes/<change-id>/plan.md`.
   - If invoked with `@context/changes/<change-id>/plan.md` or a full path, accept it.
   - **Refuse if the resolved path starts with `context/archive/`** — print "This change is archived. Open a new change with `/oracle-new` instead." and STOP.
...
1. A change-id (e.g., `/oracle-implement oauth-login phase 1`), or
...
- **Run the phase-end commit ritual**: After all automated checks pass for the phase, walk through this sequenced ritual to author one Conventional-Commits commit and write the closing short SHA back into every Progress row flipped during the phase.
  ...
  9. **Write the SHA back into Progress.** For every Progress row flipped during this phase, perform a targeted file edit:
     ...
     Skip rows that already carry a SHA suffix (resume safety: if the ritual is re-entered after a partial run, do not double-append). If `SHA=""`, skip the append entirely — the rows stay SHA-less and `/oracle-archive` will surface them as informational warnings under its missing-SHA soft-warning check.
...
- **Next phase decision**: If there is a next phase, help the user decide whether to continue or start fresh.
  ...
  - "Review this phase first" (description: "Run /oracle-impl-review to verify implementation against the plan before proceeding.")

  **If user chooses to review**: Run `/oracle-impl-review @[path-to-plan] phase [N]` to review the just-completed phase. After the review completes, re-present the continue/clear decision (without the review option this time).
  ...
  **If user chooses to clear**: Copy the resume command to clipboard and display it:
  1. Copy:
     ```bash
     echo -n "/oracle-implement <change-id> phase [next-phase-number]" | pbcopy 2>/dev/null || echo -n "/oracle-implement <change-id> phase [next-phase-number]" | clip.exe 2>/dev/null || echo -n "/oracle-implement <change-id> phase [next-phase-number]" | xclip -selection clipboard 2>/dev/null || true
     ```

     ```powershell
     # PowerShell (Windows)
     Set-Clipboard "/oracle-implement <change-id> phase [next-phase-number]"
     ```
  2. Display:
     ```
     → /oracle-implement <change-id> phase [next-phase-number] (✓ copied)
     ```
...
Empty-diff phases (manual-verification-only or no-op adapted phases) commit nothing and leave their rows SHA-less; `/oracle-archive` will surface them as informational warnings under its missing-SHA soft-warning check. This is intentional — not every phase produces code.
...
1. **Defensive pending-items surface.** Re-scan the entire `## Progress` section one last time for any `- [ ]` rows. Under normal flow this is a no-op — the trigger condition for "After all phases" is already "every `- [ ]` is `- [x]`", so the surface should find nothing. It exists to make any unexpected stragglers explicit rather than silently lost (e.g., if a partial run, a manual edit, or a resume path bypassed the trigger). If the count is non-zero, list each row as `<phase>.<index> <title>` grouped by Automated vs Manual subsection in document order, then ask the user:
   ...
   - "Proceed to epilogue" (description: "Flip status: implemented and run the epilogue commit anyway. Stragglers will surface as warnings under /oracle-archive.")
...
2. Update `change.md`: set `status: implemented`, `updated: <today>`. (Do NOT set `archived_at` — that belongs to `/oracle-archive`.)
3. Do NOT write any HTML comment progress marker at the bottom of the plan.
4. **Run the epilogue commit.** The final phase's commit cannot contain its own SHA (chicken-and-egg), so the SHA write-back into the final phase's Progress rows plus the `change.md` status flip both sit dirty in the working tree after the final phase ritual returns. Author one closing commit to land them — otherwise `/oracle-archive`'s hard-refusal gate (uncommitted paths inside the change folder) will block. Steps:
...
"Plan complete. Would you like a final implementation review?" with the following options:
  - "Run full review (/oracle-impl-review)" (description: "Comprehensive review of all phases against the plan. Catches cross-phase issues.")
  - "Skip review — I'm satisfied" (description: "No review needed. Mark the plan as done.")

If user chooses review → run `/oracle-impl-review <change-id>` (no phase number = full plan review).
